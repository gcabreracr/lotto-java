...
public class CambiarPin extends javax.swing.JInternalFrame {
    ...
    public CambiarPin() {
        ...
        valida.limitarCaracteres(txtPinActual, 8);
        valida.soloNumeros(txtNuevoPin);
        valida.limitarCaracteres(txtNuevoPin, 8);
        valida.soloNumeros(txtConfirmaPin);
        valida.limitarCaracteres(txtConfirmaPin, 8);

        usr = new UsuarioVO();
        usr.setCod_usuario(Variables.mCODUSU);
        usr.setId_usuario(Variables.mIDUSU);

        txtCodigoUsu.setText(Variables.mIDUSU);
    ...
    private void btnCambiarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCambiarActionPerformed
        ...
        usr.setPin_pass(pinActual);
        

        try {
            if (usuarioDAO.Login(usr)) {
                usr.setPin_pass(nuevoPin);
                usuarioDAO.actualizaPin(usr);
